...
class QuadElement(ElementBaseClass):
    def __init__(self, element_input_settings: dict) -> None:
        super(QuadElement, self).__init__(element_input_settings)
    ...
    def _calculate_quadrature(self) -> (jnp.ndarray, jnp.ndarray):
        if self.quadrature_order == 1 and self.shape_function_order == 1:
            ...
            xi = jnp.array([[-1.0 / jnp.sqrt(3.0), -1.0 / jnp.sqrt(3.0)],
        ...
        if self.shape_function_order == 1:
            for q in range(self.number_of_quadrature_points):
                N_xi = jax.ops.index_update(N_xi, jax.ops.index[q, :, :],
                                            0.25 * jnp.array([[(1.0 - self.xi[q, 0]) * (1.0 - self.xi[q, 1])],
                                                              [(1.0 + self.xi[q, 0]) * (1.0 - self.xi[q, 1])],
                                                              [(1.0 + self.xi[q, 0]) * (1.0 + self.xi[q, 1])],
                                                              [(1.0 - self.xi[q, 0]) * (1.0 + self.xi[q, 1])]]))
            return N_xi
        else:
    ...
    def _calculate_shape_function_gradients(self) -> jnp.ndarray:
        grad_N_xi = jnp.zeros((self.number_of_quadrature_points, self.number_of_shape_functions, 2))
        if self.shape_function_order == 1:
            for q in range(self.number_of_quadrature_points):
                grad_N_xi = jax.ops.index_update(grad_N_xi, jax.ops.index[q, :, :],
                                                 0.25 * jnp.array([[-(1.0 - self.xi[q, 1]), -(1.0 - self.xi[q, 0])],
                                                                   [(1.0 - self.xi[q, 1]), -(1.0 + self.xi[q, 0])],
                                                                   [(1.0 + self.xi[q, 1]), (1.0 + self.xi[q, 0])],
                                                                   [-(1.0 + self.xi[q, 1]), (1.0 - self.xi[q, 0])]]))
            return grad_N_xi
        else:
